namespace L13_UI {
  ...
  function hndJump(_event: KeyboardEvent): void {
    if (ƒ.Keyboard.isPressedOne([ƒ.KEYBOARD_CODE.SPACE]) && avatar.mtxLocal.translation.y <= 0) {
      jumpSpeed = 0.3;
    }
  }
  ...
  function hndLoop(_event: Event): void {
    ...
    avatar.mtxLocal.translateY(jumpSpeed);

    if (avatar.mtxLocal.translation.y <= 0) {
      jumpSpeed = 0;
      dy = 0;
    } else if (avatar.mtxLocal.translation.y > 0) {
